...
class Theme_Admin_ExtensionsModel extends Theme_ExtensionsModel
{
    public function installExtension($name)
    {
        ...
        if (isset($setting_extensions[$name])) {
            $extension = $this->tbManager->getExtension($name);
            return array(
              'success' => false,
              'message' => sprintf($tbLang->get('extension_already_installed'), $extension->getInitOption('title'))
            );
        }

        $not_installed_extensions = $this->tbManager->getNotInstalledExtensions();
        if (!isset($not_installed_extensions[$name])) {
            return array(
              'success' => false,
              'message' => sprintf($tbLang->get('extension_cannot_be_found'), $name)
            );
        }
        ...
        return array(
          'success' => true,
          'message' => sprintf($tbLang->get('extension_has_been_installed'), $extension->getInitOption('title'))
        );
    }
    ...
    public function uninstallExtension($name)
    {
        ...
        $result = array(
          'success' => true,
          'message' => sprintf($tbLang->get('extension_has_been_uninstalled'), $extension->getInitOption('title'))
        );
